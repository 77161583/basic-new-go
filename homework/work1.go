...
// ShrinkSlice 缩容机制：就是创建一个新的切片，新的容量切片要小于原始切片的容量，然后将原始切片的容量复制到新切片里面
func ShrinkSlice(slice []int, newCap int) []int {
	if newCap < 0 {
		newCap = 0 // 防止传入负值导致错误
	}
	if newCap > cap(slice) {
		newCap = cap(slice) // 不允许新的容量大于原始容量
	}
	if newCap >= len(slice) {
		// 如果新的容量大于或等于原始切片的长度，则不需要缩容
		return slice
	}

	// 创建一个新的切片，其长度和容量均为newCap
